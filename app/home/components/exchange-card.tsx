...
export default function ExchangeCard(input: CardExchangeInfoProps) {
  return (
    <Paper p="md" style={{ maxWidth: 400, margin: "auto" }}>
      <Card padding="lg" radius="md" withBorder>
        <Card.Section>
          <Image src={getImageUrl(input.acronym)} height={160} alt="Norway" />
        </Card.Section>
        ...
        <Group justify="space-between" mt="md" mb="xs">
          <Text
            ta="start"
            fw={500}
          >{`${input.code_source} => ${input.code_destination}`}</Text>
        </Group>
